...
- `test_drumgen.py` — Comprehensive test suite (pytest, 265 tests). Covers cell integrity, time signatures, style pools, assembler, humanizer, MIDI engine, end-to-end generation, probability grids, layer mode, mixed meters (including note position verification), variations, Phase 3 style palette expansion, advanced humanization (velocity contour, flam, section drift, ghost clustering, seed reproducibility), and MIDI duration overshoot verification for odd meters (3/4, 7/8, 6/8) at multiple tempos.
...
**Output:** Both CLI and GUI default to `C:\Users\%USERNAME%\Documents\drumgen_output` on WSL (auto-detected via `cmd.exe /C echo %USERNAME%`), `Documents/drumgen_output` on native Windows, or `output/` on macOS/Linux. The GUI output folder is configurable and persisted in `.drumgen_config.json`. Filenames include time signature for non-4/4 meters (e.g. `shellac_130bpm_3_4_4bars.mid`). Filenames auto-increment (`_1`, `_2`, etc.) to prevent overwriting existing files.
