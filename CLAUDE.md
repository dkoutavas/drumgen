...
# Import all .mid files in a directory (auto-detect tags from filenames)
...
python als_extractor.py project.als --drums-only
python midi_reader.py extracted/project/ --auto-tag
...
- `app.py` — Streamlit GUI. Same generation pipeline as CLI. Includes pattern preview grid rendering, optional FluidSynth audio preview, and MIDI import via sidebar expander.
...
- `cell_library.py` — All rhythmic cells defined as Python functions returning dicts. Contains `CELLS` registry, `STYLE_POOLS` (style -> list of cell names), `SECTION_PREFERENCES` (section type -> preferred tags), and lookup functions (`get_cell`, `get_pool`, `get_cell_for_section`). Also loads user-imported cells from `user_cells/` via `load_user_cells()`.
- `midi_reader.py` — Standalone CLI + importable library. Reads `.mid` files via mido, converts to drumgen's native cell format (flat 5-tuple hits), saves as JSON in `user_cells/`. Exposes `midi_to_cell()` and `save_cell()` for GUI use.
- `als_extractor.py` — Standalone CLI. Opens `.als` files (gzip-compressed XML), finds MidiClip elements from both Session and Arrangement views, writes each as a `.mid` file to `extracted/`.
...
- `midi_engine.py` — Position-to-tick math and MIDI file writing via `mido`. Constants: PPQ=480, note duration=30 ticks, MIDI channel=9.
- `preview.py` — Optional FluidSynth-based WAV rendering for the Streamlit GUI.
- `kit_mappings/` — JSON files mapping instrument names to MIDI note numbers. Default: `ugritone.json`.
...
Imported cells are stored as JSON in `user_cells/` and loaded automatically into `CELLS` at import time. They work seamlessly with `--cell`, `--list-cells`, arrangement mode, and the GUI.

**Import format:** Each JSON file contains a cell dict with `name`, `tags`, `time_sig`, `num_bars`, `humanize`, `role`, `hits` (flat 5-tuple list), `source: "imported"`, and `source_file`.

**Full pipeline:** Use `als_extractor.py` to get .mid files from Ableton projects, then `midi_reader.py` to convert them to cells. Or upload .mid files directly through the Streamlit GUI's "Import MIDI as Cell" expander.
