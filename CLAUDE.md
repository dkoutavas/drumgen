...
Plugin GUI (8-bit): Sweetie-16 palette, Press Start 2P at 8/16px, feathering off, `CornerRadius::ZERO`. 720×440 with a plugin-side resize grip (the DAW's own border cannot resize it, nih-plug limitation); the detail grid absorbs any dragged space. Elements: style picker, HUMANIZE/SWING pixel knobs, BARS/METER/FILL/SONG steppers, DICE (prime-stride seed roll) + drag-scrub SEED + SAVE .MID, the bank row (STORE toggle + 16 pads), a telegraph countdown line (`CHORUS ▸ BLAST IN 2` → `▸ BLAST NOW`; a queued pad overrides it with `▸ SLOT n NEXT BAR`), a horizon strip (current bar + next three in miniature with a sweeping playhead cursor), and the step grid (6 lanes × up to 24 sixteenths, velocity-shaded, pageable, BAR/NOW/NEXT follow modes). Repaint is 16ms while playing, 100ms stopped. If you change the window size, bump the `#[persist = "editor-state-vN"]` key or saved projects will restore the old size forever.

Pattern bank: 16 slots, each a param snapshot (style by NAME) with a pre-generated pattern, triggered by MIDI notes 36..51 or pad clicks, switch quantized to the next bar boundary. The bank is a PLAYBACK OVERLAY — it never writes params, and any USER tweak except tempo exits bank mode (tempo re-generates all slots instead, since patterns bake tempo). The host is not the user: `classify_change` in lib.rs tracks the METER param index next to the effective meter, so a host time-signature flip regenerates the live pattern and re-bakes Auto-meter pads (`plan_pump`'s auto mask) without exiting the bank, while a METER stepper press — even one that lands on the same effective meter — is a tweak and exits. Playback position is measured from `origin` (a `Drumgen` field, not on `Pattern`): normally 0, moved to the boundary on a slot switch so the new slot starts from its bar 1. Trigger gate reads the `stored` atomic mask (NOT "pattern arrived") so a press during the generation window queues instead of dying; the swap waits for the pattern. Pad colors: dark empty, faint stored-generating, solid ready, lime+border playing, blinking queued. HARD-WON: never gate the bank pump on `ProcessMode` — Bitwig reports its live engine as Offline; the pump is non-blocking try_sends and needs no bounce protection (only `await_pending` does).
...
Session log: the plugin appends one line per decision to `~/drumgen_output/drumgen.log` — transport edges, host meter flips, pad triggers (with the reject reason), swaps (with the origin tick), bank exits (with the cause), dirty causes, and every generation (`GEN`/`BUILT` with style name, bars, meters, event count; `BUILD FAILED` on a panic). The audio thread only try_sends Copy-only `LogEvent`s to the worker (drop-on-full — the log is a trace, not a ledger); the worker formats, writes, and flushes on a 250ms `recv_timeout` wake. Starts fresh past 1 MB. When debugging a field report, read the log before asking for screenshots: `tail -f ~/drumgen_output/drumgen.log` during a session shows the plugin's decisions live.
