...
- `test_drumgen.py` — Comprehensive test suite (pytest, 230 tests). Covers cell integrity, time signatures, style pools, assembler, humanizer, MIDI engine, end-to-end generation, probability grids, layer mode, mixed meters (including note position verification), variations, Phase 3 style palette expansion, and advanced humanization (velocity contour, flam, section drift, ghost clustering, seed reproducibility).
...
**Output:** MIDI files go to `output/` by default (CLI) or a configurable folder (GUI, defaults to `/mnt/c/Users/.../drumgen_output` for WSL-to-Windows access). Filenames auto-increment (`_1`, `_2`, etc.) to prevent overwriting existing files.
